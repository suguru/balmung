...
function Optimizer(config) {
  ...
  self.queue = async.queue(function(task, callback) {
    ...
    dst.mkdirp(path.dirname(file.path), function(err) {
      ...
      // copy file
      var rs = fs.createReadStream(path.join(src.path, file.path));
      var ws = fs.createWriteStream(path.join(dst.path, file.path));
      rs.pipe(ws);
      ws.on('close', function() {
        // execute optimization
        self._optimize(path.join(dst.path, file.path), function(err) {
 ...
 * Optimize target 
 */
Optimizer.prototype.optimize = function(src, dst, file) {
  this.queue.push({ src: src, dst: dst, file: file });
};
...
Optimizer.prototype._optimize = function(filePath, callback) {
  ...
  async.eachSeries(tools, function(tool, done) {
    ...
    fs.readFile(filePath, function(err, fileData) {
      ...
      var toolOptions = config[tool] || {};
      var optimizer = self[tool];
      if (optimizer) {
        optimizer(filePath, fileData, toolOptions, function(err) {
          if (err) {
            return done(err);
          }
          // check size
