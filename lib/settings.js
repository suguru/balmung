...
exports.load = function(config, callback) {
  ...
  if (config.settings) {
    ...
    if (_.isObject(config.settings)) {
      callback(null, config.settings);
    } else if (_.isString(config.settings)) {
      // Get from URL
      logger.debug('Loading settings from', { url: config.settings });
      if (config.settings.indexOf('http') === 0) {
        http.get(config.settings, function(res) {
          if (res < 200 || res.status > 299) {
            callback(new Error('Invalid http status for ' + config.settings + ' ' + res.statusCode));
          } else {
            var data = '';
            ...
            res.on('end', function() {
              try {
                rootSettings = JSON.parse(data).settings;
        ...
        logger.debug('Loading settings from', { path: config.settings });
        // Get from file
...
exports.save = function(config, callback) {
  ...
  var text = JSON.stringify(rootSettings);
  fs.writeFile(
